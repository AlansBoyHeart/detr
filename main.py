...
def get_args_parser():
    ...
    parser.add_argument('--dilation', default=False,  action='store_true', help="If true, we replace stride with dilation in the last convolutional block (DC5)")
    ...
    # Loss
    parser.add_argument('--no_aux_loss', dest='aux_loss',default=True, action='store_false',help="Disables auxiliary decoding losses (loss at each layer)")
    # * Matcher
    ...
    parser.add_argument('--coco_path', default="/data/wuxiaopeng/datasets/coco2017",type=str)
...
def main(args):
    ...
    if args.frozen_weights is not None:   #args.frozen_weights==None
    ...
    seed = args.seed + utils.get_rank()   #42+0
        ...
        if args.output_dir:
            ...
            for checkpoint_path in checkpoint_paths:
                utils.save_on_master({'model': model_without_ddp.state_dict(),  'optimizer': optimizer.state_dict(),
                                      'lr_scheduler': lr_scheduler.state_dict(), 'epoch': epoch, 'args': args, },  checkpoint_path)
